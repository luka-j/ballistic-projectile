...
class Launcher:
    ...
    def __init__(self, pitch, yaw, csv_filename, kmz_filename, dt=0.01, keep_csv=True, environment: Environment = None,
                 thrust: ThrustForce = None):
        self.pitch = pitch
        self.yaw = yaw
        self.dt = dt
    ...
    def launch(self, mass: float, position: Position, velocity=0, cross_section=lambda axis, pitch, yaw: 20,
        ...
        projectile = self.environment.create_projectile(mass, position, cross_section, drag_coeff)
        projectile.launch_at_angle(self.pitch, self.yaw, velocity)
        projectile.add_thrust(self.thrust)

        writer = CsvWriter(self.csv_filename)
        writer.write_header()
        while True:
